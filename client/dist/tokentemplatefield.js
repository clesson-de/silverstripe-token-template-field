(function () {
	...
	function Token(_ref) {
	  var token = _ref.token,
	    ...
	    colorIndex = _ref$colorIndex === void 0 ? -1 : _ref$colorIndex,
	  ...
	  var _useState = reactExports.useState(token._autoEdit || false),
	  ...
	  var typeClass = "token-template-field__token--".concat(token.type);
	  var colorClass = token.type === 'placeholder' ? colorful && colorIndex >= 0 ? "token-template-field__token--color-".concat(colorIndex) : 'token-template-field__token--mono' : '';
	  ...
	  var commitEdit = function commitEdit(value) {
	    setEditing(false);
	    if (value.length === 0) {
	      onRemove();
	    } else {
	  ...
	  return /*#__PURE__*/React.createElement("span", {
	    ...
	    onDragStart: locked ? undefined : function (e) {
	    ...
	    className: "token-template-field__token-label"
	  }, label), !locked && /*#__PURE__*/React.createElement("button", {
	    type: "button",
	...
	var uid = function uid() {
	  return "ttf-".concat(Date.now(), "-").concat(++idCounter);
	};
	...
	function serialize(tokens) {
	  return tokens.map(function (t) {
	    return t.type === 'placeholder' ? "{".concat(t.value, "}") : t.value;
	  }).join('');
	}
	function TokenTemplateField(_ref) {
	  var availableTokens = _ref.availableTokens,
	    ...
	    _ref$showFreeText = _ref.showFreeText,
	    showFreeText = _ref$showFreeText === void 0 ? true : _ref$showFreeText,
	    _ref$colorful = _ref.colorful,
	    colorful = _ref$colorful === void 0 ? false : _ref$colorful;
	  var locked = readonly || disabled;
	  ...
	  var _useState3 = reactExports.useState(null),
	    ...
	    selectedTokenId = _useState4[0],
	    setSelectedTokenId = _useState4[1];
	  ...
	  var _useState7 = reactExports.useState(false),
	    ...
	    controlsOpen = _useState8[0],
	    setControlsOpen = _useState8[1];
	  ...
	  var handleAddText = reactExports.useCallback(function () {
	    var newToken = {
	      type: 'text',
	      value: '',
	      id: uid(),
	    ...
	    setTokens(function (prev) {
	      return [].concat(_toConsumableArray(prev), [newToken]);
	    });
	  }, []);

	  // Edit a text token's value
	  var handleEditToken = reactExports.useCallback(function (id, newValue) {
	    setTokens(function (prev) {
	  ...
	  var handleAddPlaceholder = reactExports.useCallback(function (key) {
	    setTokens(function (prev) {
	  ...
	  // Remove a token
	  var handleRemove = reactExports.useCallback(function (id) {
	    setTokens(function (prev) {
	      return prev.filter(function (t) {
	        return t.id !== id;
	      });
	  ...
	  // Select a token
	  var handleSelect = reactExports.useCallback(function (id) {
	    setSelectedTokenId(id);
	  }, []);

	  // Drag & Drop handlers
	  var handleDragStart = reactExports.useCallback(function (id) {
	    setDragState({
	      draggedId: id,
	      overId: null
	    });
	  }, []);
	  var handleDragOver = reactExports.useCallback(function (e, id) {
	    e.preventDefault();
	  ...
	  var handleDrop = reactExports.useCallback(function (e, targetId) {
	    e.preventDefault();
	    if (!dragState) return;
	    ...
	    setTokens(function (prev) {
	      var draggedIndex = prev.findIndex(function (t) {
	        return t.id === draggedId;
	      });
	      var targetIndex = prev.findIndex(function (t) {
	        return t.id === targetId;
	      });
	      if (draggedIndex === -1 || targetIndex === -1) return prev;
	      var newTokens = _toConsumableArray(prev);
	      var _newTokens$splice = newTokens.splice(draggedIndex, 1),
	        _newTokens$splice2 = _slicedToArray(_newTokens$splice, 1),
	        moved = _newTokens$splice2[0];
	      newTokens.splice(targetIndex, 0, moved);
	      return newTokens;
	    });
	    setDragState(null);
	  }, [dragState]);
	  var handleDragEnd = reactExports.useCallback(function () {
	    setDragState(null);
	  }, []);

	  // Resolve label for a token
	  var getLabel = function getLabel(token) {
	    if (token.type === 'text') return token.value;
	    var entry = availableTokens[token.value];
	  ...
	  var getEntryLabel = function getEntryLabel(entry) {
	    return typeof entry === 'string' ? entry : entry.label || '';
	  };

	  // Compute a stable color index for a placeholder token key
	  var tokenKeys = Object.keys(availableTokens);
	  var getColorIndex = function getColorIndex(token) {
	    if (token.type === 'text') return -1;
	    var idx = tokenKeys.indexOf(token.value);
	    return idx >= 0 ? idx % 8 : -1;
	  };

	  // Compute color index for an available token button
	  var getButtonColorIndex = function getButtonColorIndex(key) {
	    var idx = tokenKeys.indexOf(key);
	    return idx >= 0 ? idx % 8 : -1;
	  };
	  return /*#__PURE__*/React.createElement("div", {
	    className: "token-template-field".concat(locked ? ' token-template-field--locked' : ''),
	    ref: wrapperRef,
	    ...
	    return /*#__PURE__*/React.createElement(Token, {
	      ...
	      onRemove: locked ? function () {} : function () {
	        return handleRemove(token.id);
	      },
	      onEdit: locked ? function () {} : function (newValue) {
	        return handleEditToken(token.id, newValue);
	      },
	      onDragStart: locked ? function () {} : function () {
	        return handleDragStart(token.id);
	      },
	      onDragOver: locked ? function (e) {
	        return e.preventDefault();
	      } : function (e) {
	        return handleDragOver(e, token.id);
	      },
	      onDrop: locked ? function (e) {
	        return e.preventDefault();
	      } : function (e) {
	        return handleDrop(e, token.id);
	      },
	      onDragEnd: locked ? function () {} : handleDragEnd
	    });
	  }), !locked && /*#__PURE__*/React.createElement("button", {
	    type: "button",
	    className: "token-template-field__toggle-btn".concat(controlsOpen ? ' token-template-field__toggle-btn--open' : ''),
	    onClick: function onClick() {
	      return setControlsOpen(function (prev) {
	        return !prev;
	      });
	    },
	    ...
	    className: "token-template-field__controls".concat(controlsOpen ? '' : ' token-template-field__controls--hidden')
	  }, showFreeText && /*#__PURE__*/React.createElement("button", {
	    type: "button",
	    ...
	    className: "token-template-field__token-selector"
	  }, Object.entries(availableTokens).map(function (_ref2) {
	    var _ref3 = _slicedToArray(_ref2, 2),
	      key = _ref3[0],
	      entry = _ref3[1];
	    var btnColorClass = colorful ? " token-template-field__available-token-btn--color-".concat(getButtonColorIndex(key)) : ' token-template-field__available-token-btn--mono';
	    return /*#__PURE__*/React.createElement("button", {
	      key: key,
	      type: "button",
	      className: "token-template-field__available-token-btn".concat(btnColorClass),
	      onClick: function onClick() {
	        return handleAddPlaceholder(key);
	      }
	    }, getEntryLabel(entry));
	  }))));
	}
	...
	$.entwine('ss', function ($) {
	  $('.token-template-field__wrapper').entwine({
	    onadd: function onadd() {
	      var el = this[0];
	      var availableTokens = JSON.parse(el.dataset.availableTokens || '{}');
	      var initialValue = el.dataset.value || '';
	      var isReadonly = el.dataset.readonly === '1';
	      var isDisabled = el.dataset.disabled === '1';
	      var showFreeText = el.dataset.showFreeText !== '0';
	      var colorful = el.dataset.colorful === '1';
	      ...
	      var root = createRoot(reactContainer);
	      el._reactRoot = root;
	      root.render(/*#__PURE__*/React.createElement(TokenTemplateField, {
